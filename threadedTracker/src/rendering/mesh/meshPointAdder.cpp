...
void meshPointAdder::setup(){
	
	
	testApp * APP = (testApp*)ofGetAppPtr();
	
	//andrea.loadImage("face.png");
	//tracker.setup();
	...
	ofMesh indexd = APP->MMR.stillTracker.getMeanObjectMesh();
	
	cout << indexd.getVertices().size() << endl;
	cout << "bf " << convertFromIndices(indexd).getVertices().size() << endl;
	
	addForheadToFaceMesh(indexd);
	cout << indexd.getVertices().size() << endl;
	
	ofMesh mesh = convertFromIndices(indexd);
	...
	// get the holes: 
	ofPolyline leftEye = APP->MMR.stillTracker.getMeanObjectFeature(ofxFaceTracker::LEFT_EYE);
	ofPolyline rightEye = APP->MMR.stillTracker.getMeanObjectFeature(ofxFaceTracker::RIGHT_EYE);
	ofPolyline innerMouth = APP->MMR.stillTracker.getMeanObjectFeature(ofxFaceTracker::INNER_MOUTH);
	...
	holes.push_back(rightEye.getVertices());
	holes.push_back(leftEye.getVertices());
	holes.push_back(innerMouth.getVertices());
	setup(mesh);	
...
void meshPointAdder::calcluateTriangulation(){
	...
	cout << myBasis.getVertices().size() << endl;
